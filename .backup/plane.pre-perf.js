...
(() => {
  ...
  const exhaust = Array.from({ length: 120 }, (_, k) => {
    const sp = new THREE.Sprite(new THREE.SpriteMaterial({ map: smokeTex[k % 3], transparent: true, depthWrite: false, opacity: 0 }));
    sp.visible = false;
    sp.renderOrder = 2;
    sp.userData = { life: 0, max: 1, vel: new V3(), size: 1, spin: 0, seed: Math.random() * 10 };
    scene.add(sp);
    return sp;
  });
  ...
  function spawnExhaust(worldPos, side, back, speed) {
    const sp = exhaust[exhaustCursor++ % exhaust.length];
    const d = sp.userData;
    sp.position.copy(worldPos);
    d.vel.copy(back).multiplyScalar(speed * scale)
      .addScaledVector(side, (.25 + Math.random() * .35) * scale);
    d.vel.x += (Math.random() - .5) * .18 * scale;
    d.vel.y += (.05 + Math.random() * .15) * scale;
    d.vel.z += (Math.random() - .5) * .18 * scale;
    d.life = 0;
  ...
  // ---------- Clouds ----------
  // Cumulus built from soft billboard puffs (flat, shaded base; bright, lumpy tops; a silver lining at the thin
  // edges), all drawn in one call. Anything that flies through a cloud (our plane, the dogfighters, rockets)
  // shoves the puffs aside and tears it open; the cloud slowly knits back together.
  function cloudTexture(size, seed) {
    ...
    for (let y = 0; y < size; y++) {
      for (let x = 0; x < size; x++) {
        ...
        const r = Math.hypot(u, v);
        let n = 0, amp = .5, f = 2.5;
        for (let o = 0; o < 4; o++) { n += amp * vn(x / size * f + seed, y / size * f + seed * 2); amp *= .5; f *= 2; }
        const edge = .8 + (n - .5) * .38;
        const dens = (1 - sstep(edge * .5, edge, r)) * (.82 + .18 * n);
        const k = (y * size + x) * 4;
        px[k] = 255 * n;
        px[k + 1] = px[k + 2] = 255;
  ...
  const CLOUDS = 8, PUFFS = 24, NP = CLOUDS * PUFFS;
  ...
  const cPos = new Float32Array(NP * 3), cData = new Float32Array(NP * 4);
  ...
  const cloudUni = {
    map: { value: cloudTexture(128, 5) },
    lit: { value: new THREE.Color() }, dark: { value: new THREE.Color() }, rim: { value: new THREE.Color() },
    opacity: { value: 1 },
  };
  const cloudMat = new THREE.ShaderMaterial({
    ...
    fragmentShader: `
      ...
      void main() {
        ...
        if (a < .004) discard;
        vec2 d = vUv - .5;
        float up = sin(vRot) * d.x + cos(vRot) * d.y;          // this texel's height, in world up
        float k = clamp(vShade + up * 1.15 + (t.r - .5) * .45, 0.0, 1.0);
        vec3 col = mix(dark, lit, k * k * (3.0 - 2.0 * k));
        col += rim * pow(1.0 - t.a, 3.0) * .5 * k;            // silver lining where the puff thins out
  ...
  const clouds = Array.from({ length: CLOUDS }, (_, i) => {
    ...
    for (let k = 0; k < PUFFS; k++) {
      const base = k < 7;                                   // a flat, shaded base
      const u = base ? (k / 6) * 2 - 1 : Math.random() * 2 - 1;
      const top = 1.05 * (1 - u * u) + .2;
      const y = base ? .02 + Math.random() * .08 : .15 + Math.pow(Math.random(), .75) * top;
      puffs.push({
        lx: u * 1.85 + (Math.random() - .5) * .2, ly: y, lz: (Math.random() - .5) * .95 * (1 - u * u * .5),
        size: (base ? 1 : .75 + (1 - Math.abs(u)) * .85) * (.75 + Math.random() * .45),
        shade: base ? .1 : .22 + Math.min(1, y / top) * .62,
        rot: Math.random() * 6.28, spin: (Math.random() - .5) * .06,
        alpha: base ? .8 : .88 + Math.random() * .12,
    ...
    return {
      ...
      par: .5 + Math.random() * .8, speed: .07 + Math.random() * .12, size: .75 + Math.random() * .6,
      z: -12 - (i % 4) * 5 - Math.random() * 3, cx: 0, cy: 0, S: 1, fresh: true,
    };
  ...
  function updateClouds(dt, scrollY, f) {
    ...
    for (const c of clouds) {
      ...
      for (let k = 0; k < cutters.length; k += 7) {
        const dx = cutters[k] - cx, dy = cutters[k + 1] - (cy + S * .5), dz = cutters[k + 2] - c.z;
        if (dx * dx + dy * dy + dz * dz < (reach + cutters[k + 6]) ** 2) near.push(k);
      }
      ...
      for (const q of c.puffs) {
        ...
        cData[o * 4] = q.size * S * (1 + q.tear * .45);
        cData[o * 4 + 1] = q.rot;
  ...
  function buildTrooper() {
    const mat = new THREE.MeshStandardMaterial({ vertexColors: true, roughness: .78, metalness: .06, transparent: true });
    const mesh = (geo, parent) => { const m = new THREE.Mesh(geo, mat); parent.add(m); return m; };
    ...
    scene.add(g);
    return {
      g, hips, torso, head, aim, rifle, thigh, shin, arm, elbow, flash, fire, mat,
  ...
  const troops = Array.from({ length: 16 }, buildTrooper);
  ...
  function updateTroops(dt) {
    for (const tr of troops) {
      ...
      if (tr.state === 'in') {
        ...
        tr.mat.opacity = Math.min(1, tr.t * 4);
        ...
        if (!tr.landed) {
          ...
          tr.burn += dt;
          tr.mat.color.setRGB(1, 1, 1).lerp(charCol, Math.min(1, tr.burn / .9));
  ...
  function layout() {
    ...
    buildRoutes();
    clouds.forEach(c => { c.S = c.size * scale * (mobile ? 1.15 : 1.55); c.fresh = true; });
  }
  layout();
  ...
  let time = 0, rpm = 12, puffAcc = 0, intro = 0, first = true, restK = 0, recoilS = 0;
  ...
  function freeColumn(nx, py) {
    ...
    for (const el of contentBlocks) {
      if (!el.parentElement.parentElement.classList.contains('is-live')) continue;
  ...
  function applyNight(k) {
    ...
    cloudUni.lit.value.copy(D.cloud).lerp(N.cloud, k);
    cloudUni.dark.value.copy(D.cloudDark).lerp(N.cloudDark, k);
    cloudUni.rim.value.copy(D.cloudRim).lerp(N.cloudRim, k);
    cloudUni.opacity.value = mix(D.cloudOp, N.cloudOp, k);
  ...
  function step(rawDt) {
    ...
    for (let k = 0; k < exhaust.length; k++) {
      const sp = exhaust[k];
      ...
      d.vel.multiplyScalar(1 - dt * 1.9);
      d.vel.y += .22 * scale * dt;                                // warm smoke rises
      sp.position.addScaledVector(d.vel, dt);
      sp.position.x += Math.sin(d.seed + d.life * 2.3) * .05 * scale * dt;
      sp.position.z += Math.cos(d.seed * 1.3 + d.life * 1.9) * .05 * scale * dt;
      const grow = 1 - Math.pow(1 - Math.min(1, t * 1.6), 2);
      sp.scale.setScalar(scale * d.size * (.16 + grow * 1.05 + t * .5));
      sp.material.rotation += d.spin * dt;
      const fadeIn = Math.min(1, t * 14);
      sp.material.opacity = fadeIn * Math.pow(1 - t, 1.6) * (.42 - night.k * .08);
      sp.material.color.copy(night.k > .5 ? sootNight : sootDay).lerp(smokeLight, Math.min(1, t * 2.2));
    }
