...
(() => {
  ...
  function planRaid(jpAt, a) {
    ...
    function kill(jpK, side, livery, rocket) {
      const ahead = 6.5 * s, out = { w: true, x: 0, y: 0, z: 0 }, tan = new V3();
      let wide = 1;
      // a tail chase: the fighter holds station a fixed distance ahead along our heading, so it stays in the sight
      const posAt = (jp, v) => {
        ourAt(jp, v);
        jump.curve.getTangentAt(cruiseEase(clamp(jp, 0, 1)), tan);
        v.addScaledVector(tan, ahead);
