  ...
  function create(api) {
    ...
    function stepActor(a, T, dt) {
      ...
      if (a.mode === 'fly') {
        ...
        a.roll = Math.sin(T * 1.3 + a.seed) * .22;
      } else {
      ...
      // ghost out while passing behind the page content, so text stays readable
      fB.copy(a.pos).project(camera);
      const sx = (fB.x + 1) / 2 * innerWidth, sy = (1 - fB.y) / 2 * innerHeight;
      const pad = 36;
      const behind = !!contentBox && sx > contentBox.left - pad && sx < contentBox.right + pad && sy > contentBox.top - pad && sy < contentBox.bottom + pad;
    ...
    function arenaBounds() {
      ...
      arena.bounds = isMobile() ? [find(-.8), find(.8)] : [find(.06), find(.92)];
    }
    ...
    function spawnEnemy() {
      ...
      a.nextShot = a.tb * rnd(.3, .5);
      a.shots = 0;
    }
    function updateArena(dt, playing) {
      if (!playing) return;
      if (!arena.on) {
        arena.on = true;
        arena.t = 0;
    ...
    function update(dt, settled, section, play) {
      ...
      for (const a of actors) {
        ...
        if (a.enemy) a.clock += dt;
        stepActor(a, a.enemy ? a.clock : current ? current.T : 0, dt);
      ...
      for (const tr of tracers) {
        ...
        if (d.life >= d.max) {
          ...
          if (d.player && api.onPlayerHit) api.onPlayerHit(fA.copy(d.start).addScaledVector(d.dir, d.speed * d.max));
          continue;
        }
    ...
    return { update, actors, game: gameApi, get kind() { return current ? current.kind : null; }, get time() { return current ? current.T : 0; } };
  }
