...
(() => {
  ...
  const CLOUD_LOOK = {
    day: { lit: 0xFFFFFF, dark: 0xB4C3DB, sky: 0xF2E6D2, shadow: 0x8A6E55, shadowA: .16, op: .9, light: [-.55, .84] },
    night: { lit: 0xB8C5E8, dark: 0x3C4870, sky: 0x0B101D, shadow: 0x000000, shadowA: .36, op: .88, light: [.6, .8] },
  };
  // Pass 1: puffs (round bumps, flat base ellipses, tapered tails) only lay down soft density, plus the height of
  // each pixel within its cloud. Overlapping puffs saturate into one solid body, and since the height depends
  // only on where the pixel is in the cloud, neighbouring puffs agree on it: no seams, no per-ball shading.
  const cloudUni = {};
  const cloudMat = new THREE.ShaderMaterial({
    ...
    vertexShader: `
      ...
      attribute vec4 iExtra;           // stretch (x), cloud base height, haze, shape (0 bump, 1 base, 2 tail)
      varying vec2 vUv;
      varying float vA;
      varying float vH;
      varying float vHaze;
    ...
    fragmentShader: `
      ...
      void main() {
        ...
        vec2 q;
        if (vShape < .5) q = p / .48;
        else if (vShape < 1.5) q = p / vec2(.49, .3);
        else q = vec2(p.x / .49, (p.y + p.x * p.x * .25) / (.085 * mix(1.0, .3, clamp(p.x + .5, 0.0, 1.0))));
        float a = (1.0 - smoothstep(.93, 1.0, length(q))) * vA;
        if (a < .01) discard;
        gl_FragColor = vec4(clamp(vH, 0.0, 1.0), vHaze, 0.0, a);
      }`,
  });
  ...
  // Pass 2 turns the density into one solid cloud: a smooth silhouette, one flat colour through the middle
  // (white on top, blue-grey towards the base) and 3D shading only around the rim, where the density falls
  // off: the edge facing the sun (or moon) catches the light, the far edge rolls into shade. It is a little
  // see-through and lays a soft drop shadow below.
  ...
  const cloudComp = new THREE.ShaderMaterial({
    uniforms: {
      ...
      shadow: { value: new THREE.Color() }, shadowA: { value: .2 }, opacity: { value: .9 },
    ...
    fragmentShader: `
      ...
      uniform vec2 px;
      uniform vec3 lit, dark, sky, shadow;
      uniform float shadowA;
      uniform float opacity;
      uniform vec2 light;
      varying vec2 vUv;
      float dens(vec2 uv) { return texture2D(tex, uv).a; }
      float cover(vec2 uv) { return smoothstep(.3, .62, dens(uv)); }
      void main() {
        vec4 t = texture2D(tex, vUv);
        float a = smoothstep(.3, .62, t.a) * opacity;
        vec2 o = vUv - px * vec2(3.0, -8.0);             // light from above: the shadow falls below
        float sh = (cover(o) * 2.0 + cover(o + px * vec2(4.0, 0.0)) + cover(o - px * vec2(4.0, 0.0))
          + cover(o + px * vec2(0.0, 4.0)) + cover(o - px * vec2(0.0, 5.0))) / 6.0;
        float sa = sh * shadowA * (1.0 - a);
        vec3 col = vec3(0.0);
        if (t.a > .002) {
          // rim normal from the density slope (zero inside, where the density is saturated)
          vec2 g = vec2(cover(vUv + px * vec2(7.0, 0.0)) - cover(vUv - px * vec2(7.0, 0.0)),
                        cover(vUv + px * vec2(0.0, 7.0)) - cover(vUv - px * vec2(0.0, 7.0)));
          vec3 n = normalize(vec3(-g * 1.1, 1.0));
          vec3 L = normalize(vec3(light, .9));
          float rim = dot(n, L) - L.z;
          float h = clamp(t.r / t.a, 0.0, 1.0);
          float l = clamp(.42 + .58 * smoothstep(.0, .6, h) + rim * 1.15, 0.0, 1.0);
          col = mix(mix(dark, lit, l), sky, clamp(t.g / t.a, 0.0, 1.0));
        }
        gl_FragColor = vec4(col * a + shadow * sa, a + sa);
      }`,
    depthTest: false,
    depthWrite: false,
    toneMapped: false,
  ...
  function tintClouds(k) {
    ...
    ['lit', 'dark', 'sky', 'shadow'].forEach(key => U[key].value.setHex(D[key]).lerp(lookTmp.setHex(N[key]), k));
    U.shadowA.value = D.shadowA + (N.shadowA - D.shadowA) * k;
  ...
  // A rounded cumulus: flat ellipses along the base, a row of round bumps rising to a crown in the middle,
  // a few bumps in front (their white scallops break up the shaded body) and a tapered tail or two.
  const clouds = Array.from({ length: CLOUDS }, (_, i) => {
    const puffs = [];
    const R = (a, b) => a + Math.random() * (b - a);
    const add = (lx, ly, lz, size, lit, shape, stretch = 1, rot = 0) => puffs.push({
      lx, ly, lz, size, lit, shape, stretch, rot, spin: 0, alpha: 1, seed: Math.random() * 10,
      dx: 0, dy: 0, dz: 0, vx: 0, vy: 0, vz: 0, tear: 0,
    });
    const lean = R(-.25, .25);                          // where the crown sits
    for (let k = 0; k < 3; k++) add((k - 1) * .85, .55, -.7, 1.2, 1, 0);       // a solid core
    for (let k = 0; k < 6; k++) add(-1.55 + k * .62 + R(-.08, .08), .08, R(-.6, -.3), R(.8, 1.0), 0, 1, 1.35);
    for (let k = 0; k < 10; k++) {
      const u = -1 + k / 9 * 2 + R(-.05, .05);
      const x = u * 1.55, crown = 1 - Math.pow(Math.abs(u - lean * .6), 1.6);
      add(x, .25 + crown * R(.6, .9), R(-.45, -.1), (.6 + crown * .95) * R(.85, 1.1), 1, 0);
    }
    for (let k = 0; k < 6; k++) {
      const u = R(-.85, .85);
      add(u * 1.5, R(.12, .42), R(.05, .5), R(.45, .75) * (1 - Math.abs(u) * .3), .75, 0);
    }
    const tails = Math.random() < .75 ? (Math.random() < .35 ? 2 : 1) : 0;
    const side = Math.random() < .5 ? -1 : 1;
    for (let k = 0; k < 2; k++) {
      const s = k ? -side : side, on = k < tails;
      add(s * R(2.15, 2.5), R(.1, .22), R(.1, .4), R(1.3, 1.8), 1, 2, 1, s > 0 ? 0 : Math.PI);
      if (!on) puffs[puffs.length - 1].alpha = 0;
    }
  ...
  function planRaid(jpAt, a) {
    ...
    // Kill: sweeps in from the side, settles ahead of us (above and off to one side of our line), takes hits
    // (sparks, then smoke) and goes down: blown apart, or burning and falling away from our path
    function kill(jpK, side, livery, rocket, how) {
      const lead = .9 / D, out = { w: true, x: 0, y: 0, z: 0 };
      let wide = 1;
      const posAt = (jp, v) => {
        ourAt(jp + lead, v);
        const k = 1 - smooth(jpK - 1.7 / D, jpK - .55 / D, jp);
        const hh = (camera.position.z - v.z) * TAN_HALF, hw = hh * camera.aspect;
        v.x += side * (1.3 * s * wide + 1.45 * hw * k);
        v.y += 1.9 * s * wide + .25 * hh * k;
        v.z += 2 * k;
        return v;
      };
      ...
      const down = () => {
        ...
        df.kill(foe, how);
        if (how === 'burn') foe.vel.add(fxB.set(side * 3.5 * s, -1.2 * s, 1.5 * s));
      };
    ...
    kill(jpAt('k1'), -a, liv[0], false, 'blast');
    kill(jpAt('k2'), a, liv[1], true, 'blast');
    kill(jpAt('k3'), -a, liv[2], false, 'burn');
    kill(jpAt('k4'), a, liv[3], true, 'blast');
    return events;
  }
  ...
  function fireGun(gn, aim) {
    ...
    d.dir.normalize();
    if (aim) {
      // Jump battle: straight at the fighter ahead, with a little spread
      d.dir.subVectors(aim, d.start);
      const dist = d.dir.length() || 1;
      d.dir.divideScalar(dist);
      d.dir.x += (Math.random() - .5) * .025;
      d.dir.y += (Math.random() - .5) * .025;
      d.dir.z += (Math.random() - .5) * .025;
      d.dir.normalize();
      d.speed = 50 * scale;
      d.len = .8 * scale;
  ...
  function step(rawDt) {
    ...
    salvoTimer -= dt;
    updateCrew(dt, parked, f, gunsHot || gunGlow > .05, jumping && J.p > jump.phase[0] && J.p < jump.phase[2]);
    ...
    if (hudTimer > .08) {
      ...
      if (jumping && !onWheels && groundW < .05) {
        ...
        status = jp < ph[0] ? 'Climbing out' : jp < ph[1] ? 'Evading' : jp < ph[2] ? 'Engaging' : 'Inbound';
      }
